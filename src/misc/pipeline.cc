    ...
    std::atomic<unsigned long>& current_idx)
{
    ...
    std::cout << "Sample " << this_idx << " output: '" << input.second << "' finished at " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - start_time).count() << std::endl;
